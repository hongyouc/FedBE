...
class BasicBlock(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1, option='A'):
        ...
        self.conv2 = nn.Conv2d(planes, self.planes, kernel_size=3, stride=1, padding=1, bias=False)
    ...
    def forward(self, x):
        ...
        if self.stride != 1 or self.in_planes != self.planes:
          residual = F.pad(x[:, :, ::2, ::2], (0, 0, 0, 0, self.planes//4, self.planes//4), "constant", 0)
          out += residual
        out = F.relu(out)
        return out
